...
# Add PIL import for resampling constants
...
class FakeTextDataGenerator:
    ...
    def generate(
        ...
        # ----1. 生成文字图 (整行)----
        #    image_generator.generate 返回 (text_img, box_info)
        text_img, _ = image_generator.generate(
            out_dir,
            text=text,
            font_list=font_list,
        ...
        # 使用 BICUBIC 插值以减少锯齿感
        rotated_text_img = text_img.rotate(angle, resample=Image.BICUBIC, expand=True)
        ...
        final_img = img_aug.apply_augmentations(background_img)
